...
export default function MenuItem({submenu, link, name,subMenuState, subMenuSetState, condition }) {
  ...
  return (
    ...
    <li  className={`font-semibold text-sm px-2 py-3 cursor-pointer transition-colors  duration-300    text-black
    ${
    ...
    >
      <section className="flex items-center justify-between">
        <section className="flex">
          <div className={`text-xl`}><Icons iconType="home"/></div>
            <div className={`pl-2`}>{name}</div>
        </section>
       {submenu &&  <section className={`${ subMenuState ? "rotate-90" : ""}`}><Icons iconType="rightArrow"/></section>}
      </section>
    </li>
  </Link>
  )
